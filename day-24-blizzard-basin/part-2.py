...
def parse_thing():
    _, blizdex, xrow, xcol, _ = next_Step_Re.split(next_step)
    return int(blizdex), int(xrow), int(xcol)
...
print(f'\nFirst Path Len {shortest_path}')
answer += shortest_path

# SECOND - do it all again in reverse
...
print(f'Second Path Len {shortest_path}')
answer += shortest_path

# THIRD - do it all again in reverse in reverse
start_node = paths[-1][-1]
distances, previous = dijkstra_shortest_path(input_graph, start_node)
shortest_path = float('inf')
for end_node in end_nodes:
...
print(f'Third Path Len {shortest_path}')
answer += shortest_path
